    ...
    if ($do == "add" && explode("_",$router)[0] == "sess") {
		...
		if (is_file($config_file) && !is_writable($config_file)) {
	    		...
	    		echo json_encode($mess);
		  		...
		  		foreach (file('../config/config.php',FILE_SKIP_EMPTY_LINES) as $line) {
			      $ss = explode("'", $line)[1];
			      $useradm = get_config($line,'skynity<|<', "'");
			      $passadm = get_config($line,'skynity>|>', "'");
			      
			      if ($ss == "skynity") {
			      	

				    $cari = array('1' => "skynity<|<$useradm", "skynity>|>$passadm");
				    $ganti = array('1' => "skynity<|<$suseradm", "skynity>|>$spassadm");
