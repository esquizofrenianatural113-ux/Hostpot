...
if(isset($_POST['sessname']) && isset($_SESSION["skynity"])){
    ...
    $API->connect($iphost, $userhost, dec_rypt($passwdhost));
  	...
  	$get_expire_mon = $API->comm("/system/scheduler/print", array(
            "?name" => "Skynity-Expire-Monitor",));

    if($get_expire_mon[0]['name'] !== "Skynity-Expire-Monitor"){
      $expmon = $API->comm("/system/scheduler/add", array(
        ...
        "disabled" => "no",
        "comment" => "Skynity Expire Monitor",
        ));
